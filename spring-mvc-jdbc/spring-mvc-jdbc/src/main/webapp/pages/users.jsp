...
<body>
	...
	<div><a href="http://localhost:8080/spring-mvc-jdbc/addUser">Add</a>
				...
				<tbody>
					<c:forEach var="user" items="${userDetail}">
						<tr>
							...
							<td>${user.lastName}</td>
							<td>${user.email}</td>
							<td><fmt:formatDate  pattern="dd/MM/yyyy" value="${user.dob}" type="date" /></td>
							<td><a
								href="<%=request.getContextPath()%>/update/${user.id}">Update</a>
								&nbsp; <a
